...
export default function SignatureResizer() {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      if (smartFix && (preset?.signature || (targetW && targetH))) {
        const options = preset?.signature
          ? preset
          : {
              width: targetW,
              height: targetH,
        ...
        if (!targetW || !targetH) {
          throw new Error('Please enter width and height, or select an exam preset.');
        }
  ...
  const handleDownload = () => {
    ...
    a.download = `resized_signature_${Date.now()}.${ext}`;
    a.click();
  };
  ...
  return (
    <div className="page-container sig-resizer-page">
      <SEOHead
        title="Signature Resizer Online Free – Bank SSC IBPS UPSC Forms"
        path="/signature-resizer"
        keywords="signature resizer, resize signature, signature resize for forms, SSC signature size, IBPS signature size, NEET signature size, free signature resizer"
        description="Free signature resizer for Indian bank and exam forms. Exact pixel sizes for SSC, IBPS, SBI, UPSC, NEET. Clean ink, white background, private browser tool."
      />

      <header className="page-header">
        <h1>Free Signature Resizer for Bank & Exam Forms</h1>
        <p className="subtitle">
          Resize and clean your signature to the exact pixel size required by SSC, IBPS, SBI, UPSC,
          NEET and other Indian forms. Get a clear ink signature on a white background in one click.
          Fully private — processing stays in your browser.
        </p>
      </header>
          ...
          <div className="control-group ai-fix-toggle">
            <label className="ai-toggle-label">
              ...
              <span className="ai-toggle-text">
                ...
                <small>Clean ink, remove noise, white bg &amp; fit exact size</small>
              </span>
            </label>
          </div>
          ...
          {!smartFix && (
            <div className="control-group">
              ...
              <div className="format-toggle">
                <button
                  type="button"
                  className={bgColor === '#ffffff' ? 'active' : ''}
                ...
                <button
                  type="button"
                  className={bgColor === 'transparent' ? 'active' : ''}
        ...
        <div className="preview-column">
          {!originalPreview ? (
            <div className="preview-placeholder glass-panel">
              ...
              <p>Upload a signature to see preview</p>
            </div>
          ) : (
            <div className="preview-card glass-panel">
              ...
              <div className="preview-image-container checkerboard">
                <img src={originalPreview} alt="Original signature" className="sig-img" />
              </div>
          ...
          {resultPreview && (
            <>
              ...
              <div className="preview-card glass-panel result-card">
                <div className="preview-header success-text">Resized Signature</div>
                <div className="preview-image-container checkerboard">
                  <img src={resultPreview} alt="Processed signature" className="sig-img" />
                </div>
                <div className="preview-badges">
                  {resultDims && (
                    <span className="badge highlight dim-badge">
      ...
      <div className="tips-section glass-panel mt-4">
        <h3>💡 Tips for a Perfect Signature</h3>
        <ul>
          <li>Use a thick black or dark blue pen on unlined white paper.</li>
          <li>Ensure good lighting without harsh shadows when taking a photo.</li>
          <li>Crop the image tightly around the signature before uploading if possible.</li>
          <li>
            Many forms require specific dimensions (e.g. 140×60 px for SSC / IBPS). Select the
            exam preset above for exact sizes.
          </li>
          <li>
            Enable <strong>AI Auto-Fix</strong> to automatically clean noise, force white
            background, and fit the signature cleanly inside the required box.
          </li>
        </ul>
      </div>
    </div>
