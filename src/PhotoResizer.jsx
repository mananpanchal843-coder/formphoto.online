...
import { resizeImage, resizeImageSmart, smartFixPhoto, getImageDimensions, formatFileSize } from './imageProcessor';
      ...
      <div className="tool-layout">
        <div className="controls-column glass-panel">
          ...
          <div className="control-group">
            <PresetSelector type="photo" selectedPreset={getPresetById(presetId)} onSelect={(preset) => setPresetId(preset.id)} />
          </div>
          <div className="control-group ai-fix-toggle">
            <label className="ai-toggle-label">
              <input type="checkbox" checked={smartFix} onChange={(e) => setSmartFix(e.target.checked)} />
              <span className="ai-toggle-text"><strong>✨ AI Auto-Fix</strong><small>Enhance, clean background, smart crop &amp; match file size</small></span>
            </label>
          </div>
          <div className="dimensions-row">
            <div className="input-wrapper"><label>Width (px)</label><input type="number" value={width} onChange={e => setWidth(e.target.value)} placeholder="e.g. 350" /></div>
            <div className="input-wrapper"><label>Height (px)</label><input type="number" value={height} onChange={e => setHeight(e.target.value)} placeholder="e.g. 450" /></div>
          </div>
          <div className="control-group">
            <label>Background Color</label>
            <div className="color-swatches">
              {['#ffffff', '#000000', '#f0f0f0', '#0000ff', '#ff0000'].map(c => (
                <button type="button" key={c} className={`color-swatch ${bgColor === c ? 'active' : ''}`} style={{ backgroundColor: c }} onClick={() => setBgColor(c)} title={c} aria-label={`Select color ${c}`} />
              ))}
          ...
          <div className="control-group">
            ...
            <div className="format-toggle">
              <button type="button" className={format === 'jpeg' ? 'active' : ''} onClick={() => setFormat('jpeg')}>JPEG</button>
              <button type="button" className={format === 'png' ? 'active' : ''} onClick={() => setFormat('png')}>PNG</button>
            </div>
          </div>
          <div className="sticky-action-bar">
            <button type="button" className={`primary-button large-button process-btn ${processing ? 'processing' : ''}`} onClick={handleProcess} disabled={!file || processing}>
              {processing ? <span className="spinner-text"><span className="spinner" /> Processing...</span> : 'Resize Photo'}
            </button>
          </div>
        ...
        <div className="preview-column">
          {!originalPreview ? (
            <div className="preview-placeholder glass-panel"><div className="placeholder-icon">📸</div><p>Upload a photo to see preview</p></div>
          ) : (
            <div className="preview-card glass-panel">
              <div className="preview-header">Original</div>
              <div className="preview-image-container"><img src={originalPreview} alt="Original preview" /></div>
          ...
          {resultPreview && (
            <>
              <div className="arrow-divider"><div className="arrow-icon">↓</div></div>
              <div className="preview-card glass-panel result-card">
                <div className="preview-header success-text">Resized Result</div>
                <div className="preview-image-container checkerboard"><img src={resultPreview} alt="Processed result" /></div>
                <div className="preview-badges">
                ...
                <div className="action-row"><a href={resultPreview} download={`resized_photo.${format}`} className="download-button">Download Photo</a></div>
              </div>
            </>
          )}
        </div>
